...
namespace TrueCareer.Services.MAppUser
{
    ...
    public class AppUserService : BaseService, IAppUserService
    {
        ...
        private void Sync(List<AppUser> AppUsers)
        {
            ...
            FavouriteMentors.AddRange(AppUsers.SelectMany(x => x.FavouriteMentorMentors.Select(y => new FavouriteMentor { Id = y.Id })));
            FavouriteMentors.AddRange(AppUsers.SelectMany(x => x.FavouriteMentorUsers.Select(y => new FavouriteMentor { Id = y.Id })));
            FavouriteNews.AddRange(AppUsers.SelectMany(x => x.FavouriteNews.Select(y => new FavouriteNews { Id = y.Id })));
            News.AddRange(AppUsers.SelectMany(x => x.FavouriteNews.Select(y => new News { Id = y.NewsId })));
            Information.AddRange(AppUsers.SelectMany(x => x.Information.Select(y => new Information { Id = y.Id })));
            InformationTypes.AddRange(AppUsers.SelectMany(x => x.Information.Select(y => new InformationType { Id = y.InformationTypeId })));
            Topics.AddRange(AppUsers.SelectMany(x => x.Information.Select(y => new Topic { Id = y.TopicId })));
            
            Sexes = Sexes.Distinct().ToList();
            FavouriteMentors = FavouriteMentors.Distinct().ToList();
            FavouriteNews = FavouriteNews.Distinct().ToList();
            ...
            RabbitManager.PublishList(FavouriteMentors, RoutingKeyEnum.FavouriteMentorUsed.Code);
            RabbitManager.PublishList(FavouriteNews, RoutingKeyEnum.FavouriteNewsUsed.Code);
            RabbitManager.PublishList(News, RoutingKeyEnum.NewsUsed.Code);
            RabbitManager.PublishList(Information, RoutingKeyEnum.InformationUsed.Code);
