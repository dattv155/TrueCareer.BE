...
namespace TrueCareer.Rpc.mentor_connection
{
    ...
    {
        [Route(MentorConnectionRoute.SingleListConnectionType), HttpPost]
        public async Task<List<MentorConnection_ConnectionTypeDTO>> SingleListConnectionType([FromBody] MentorConnection_ConnectionTypeFilterDTO MentorConnection_ConnectionTypeFilterDTO)
        {
        ...
        [Route(MentorConnectionRoute.SingleListAppUser), HttpPost]
        public async Task<List<MentorConnection_AppUserDTO>> SingleListAppUser([FromBody] MentorConnection_AppUserFilterDTO MentorConnection_AppUserFilterDTO)
        {
            if (!ModelState.IsValid)
                throw new BindException(ModelState);
